...
describe("buildLeadPayloadFromFormData", () => {
  ...
  it("preserves valuation-only fields in the message payload", () => {
    const payload = buildLeadPayloadFromFormData(
      formDataFrom({
        full_name: "Jan Kowalski",
        phone: "+48 600 000 000",
    ...
    expect(payload.message).toContain("Dodatkowe dane formularza:");
  ...
  it("preserves contact-only fields in the message payload", () => {
    const payload = buildLeadPayloadFromFormData(
      formDataFrom({
        full_name: "Ewa Test",
        phone: "+48600111222",
