...
describe("buildLeadPayloadFromFormData", () => {
  ...
  it("preserves valuation-only fields inside the message payload", () => {
    const payload = buildLeadPayloadFromFormData(
      formDataFrom({
    ...
    expect(payload.message).toBe(
      [
        ...
        "timeline: 1-3 miesiace",
        "",
        "Dodatkowe informacje: Prosze o szybki kontakt.",
      ].join("\n"),
    );
  });

  it("preserves contact-only fields inside the message payload", () => {
    const payload = buildLeadPayloadFromFormData(
      formDataFrom({
        full_name: "Ewa Test",
        phone: "+48600111222",
    ...
    expect(payload.message).toBe(["Szczegoly formularza:", "preferred_contact_time: 10:00-12:00"].join("\n"));
  });
});
