...
const WYCENA_MESSAGE_DETAIL_FIELDS = [
  ["district", "Dzielnica"],
...
function buildMessage(formData: FormData, leadType: string): string | undefined {
  const baseMessage = asText(formData, "message");
  if (leadType !== "wycena") {
    return baseMessage || undefined;
  }

  const details = WYCENA_MESSAGE_DETAIL_FIELDS.flatMap(([key, label]) => {
  ...
  const valuationDetails = `Szczegoly wyceny: ${details.join("; ")}`;
  return [baseMessage, valuationDetails].filter(Boolean).join("\n\n");
}
...
export function buildLeadPayloadFromFormData(formData: FormData): LeadPayload {
  ...
  return {
    ...
    message: buildMessage(formData, leadType),
