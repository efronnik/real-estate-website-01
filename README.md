...
- `STRAPI_API_TOKEN` (server-only Strapi API token with permission to create leads)
...
- Lead creation goes through the Next.js `/api/leads` proxy, which validates submissions and writes to Strapi with `STRAPI_API_TOKEN`
